...
| [RDS / Aurora](docs/services/rds.md) | ✅ Core | form-encoded body, `Version=2014-10-31` | Subnet group, parameter group, cluster, and instance CRUD — cluster endpoint resolves to a real Postgres sidecar; status always `available`; Describe `Filters` supported (`db-instance-id`, `db-cluster-id`); Performance Insights attributes round-trip (`DbiResourceId`, `PerformanceInsightsEnabled`, KMS key, retention) |
